...
import {
  ...
  RotateCw, Download, ExternalLink, AlertCircle,
} from "lucide-react";
...
export default function ArticleDetailPage() {
  ...
  const handleReprocess = async () => {
    ...
    try { await reprocessArticle(articleId); }
    catch { /* ignore */ }
    ...
    return (
      <Card className="border-destructive/50">
        <CardContent className="flex flex-col items-center py-12 gap-3">
          ...
          <CardDescription>ID {articleId} does not exist or was deleted.</CardDescription>
        </CardContent>
      </Card>
    );
  }
            ...
            <CardContent className="py-4">
              {markdown ? (
                ...
                <div className="flex flex-col items-center py-12 text-muted-foreground gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={handleReprocess}>Try Reprocessing</Button>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex gap-2">
                <a href={`${API_BASE}/articles/${articleId}/export/json`} target="_blank" rel="noopener noreferrer">
                  <Button variant="outline" size="sm" className="gap-1"><Download className="h-3.5 w-3.5"/>JSON</Button>
                </a>
                <a href={`${API_BASE}/articles/${articleId}/export/markdown`} target="_blank" rel="noopener noreferrer">
                  <Button variant="outline" size="sm" className="gap-1"><Download className="h-3.5 w-3.5"/>Markdown</Button>
                </a>
                <a href={`${API_BASE}/articles/${articleId}/export/bibtex`} target="_blank" rel="noopener noreferrer">
                  <Button variant="outline" size="sm" className="gap-1"><Download className="h-3.5 w-3.5"/>BibTeX</Button>
                </a>
              </div>
            </CardHeader>
            <CardContent>
                  ...
                  <div className="space-y-4 text-sm">
                    {extraction.abstract && (
                      <div>
                        ...
                        <p className="text-muted-foreground">{String(extraction.abstract)}</p>
                      </div>
                    )}
                    {Array.isArray(extraction.authors) && extraction.authors.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {(extraction.authors as string[]).map((a: string, i: number) => (
                      ...
                      {[
                        ...
                        val ? (
                          <div key={label}>
                            ...
                            <p className="text-muted-foreground text-xs break-all">{String(val)}</p>
                          </div>
                        ) : null
                      )}
                    </div>
                    {[
                      ...
                      ["Future Work", extraction.future_work],
                    ].map(([label, val]) =>
                      val ? (
                        <div key={label}>
                          <h4 className="font-semibold mb-1">{label}</h4>
                          <p className="text-muted-foreground">{String(val)}</p>
                        </div>
                      ) : null
                    )}
                    {Array.isArray(extraction.key_claims) && (extraction.key_claims as { claim: string }[]).length > 0 && (
                      <div>
                        <h4 className="font-semibold mb-1">Key Claims</h4>
                        <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                          {(extraction.key_claims as { claim: string }[]).map((c: { claim: string }, i: number) => (
                            <li key={i}>{c.claim}</li>
                          ))}
                        </ul>
                      </div>
                ...
                <div className="space-y-3 pr-3">
                  {messages.length === 0 && (
                    <p className="text-muted-foreground text-sm text-center py-8">
                      Ask a question about the article content, methodology, or results.
                    </p>
                  )}
                  {messages.map((msg, i) => (
                        ...
                        {msg.role === "assistant" && citations(msg).length > 0 && (
                          <div className="mt-2 pt-2 border-t border-border/50">
                            ...
                            {citations(msg).map((c, ci) => (
                              <div key={ci} className="text-xs opacity-70 mt-0.5">
                                §{c.section_title} {c.page_start ? `p.${c.page_start}` : ""} — &ldquo;{c.snippet?.slice(0, 80)}...&rdquo;
                              </div>
                            ))}
                          </div>
                        )}
        ...
        <TabsContent value="graph">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Entities and relationships extracted from the article</CardDescription>
            </CardHeader>
            <CardContent>
              {graph ? (
                <div className="space-y-4">
                  <div>
                    <h4 className="font-semibold mb-2">Entities ({graph.entities.length})</h4>
                  ...
                  <Separator />
                  <div>
                    <h4 className="font-semibold mb-2">Relationships ({graph.relationships.length})</h4>
            ...
            <CardContent>
              <div className="space-y-2 text-sm">
                {[
                  ...
                  ["Source Type", article.source_type.toUpperCase()],
                  ["Status", article.status],
                  ["Created", new Date(article.created_at).toLocaleString()],
                  ["Updated", new Date(article.updated_at).toLocaleString()],
                  ["Export", (
                    <div className="flex gap-2">
                      <a href={`${API_BASE}/articles/${articleId}/export/json`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-xs">JSON</a>
                      <a href={`${API_BASE}/articles/${articleId}/export/markdown`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-xs">Markdown</a>
                      <a href={`${API_BASE}/articles/${articleId}/export/bibtex`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline text-xs">BibTeX</a>
                    </div>
                  )],
                ].map(([label, value]) => (
