...
interface Settings {
  ai_provider: string; openai_api_key: string; openai_model: string;
  openai_embedding_model: string; anthropic_api_key: string; anthropic_model: string;
  custom_api_base: string; custom_api_key: string; custom_model: string;
  use_mock_ai: boolean; max_upload_mb: number; host: string; port: number; env_path: string;
}
...
export default function SettingsPage() {
  ...
  const [customKey, setCustomKey] = useState(""); const [customKeyTouched, setCustomKeyTouched] = useState(false);
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      setSettings(d); setProvider(d.ai_provider);
      setOpenaiKey(d.openai_api_key); setOpenaiModel(d.openai_model); setOpenaiEmbeddingModel(d.openai_embedding_model);
      setAnthropicKey(d.anthropic_api_key); setAnthropicModel(d.anthropic_model);
      setCustomBase(d.custom_api_base); setCustomKey(d.custom_api_key); setCustomModel(d.custom_model);
  ...
  const handleSave = async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        ai_provider: provider, openai_model: openaiModel, openai_embedding_model: openaiEmbeddingModel,
        anthropic_model: anthropicModel, custom_api_base: customBase, custom_model: customModel,
        use_mock_ai: mockAi, max_upload_mb: maxUploadMb,
      };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
              ...
              {PROVIDERS.map((p) => (
                ...
                >
                  <input type="radio" name="provider" value={p.value}
                    checked={provider === p.value} onChange={(e) => setProvider(e.target.value)} className="mt-1" />
            ...
            {(provider === "openai" || provider === "custom_openai") && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="pl-4 border-l-2 border-primary/30 space-y-3">
                {provider === "openai" ? (
                  <>
                    ...
                    <div className="space-y-1.5"><Label>Embedding Model</Label>
                  ...
                  <>
                    <div className="space-y-1.5"><Label>API Base URL</Label><Input value={customBase} onChange={(e) => setCustomBase(e.target.value)} placeholder="http://localhost:11434/v1"/></div>
                    <div className="space-y-1.5"><Label>API Key</Label><Input type="password" value={customKey} onChange={(e) => { setCustomKey(e.target.value); setCustomKeyTouched(true); }} placeholder="ollama or your-key"/></div>
                    <div className="space-y-1.5"><Label>Model Name</Label><Input value={customModel} onChange={(e) => setCustomModel(e.target.value)} placeholder="llama3.1:8b"/></div>
                  </>
                )}
              </motion.div>
            )}
            ...
            {(provider === "anthropic" || provider === "custom_anthropic") && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="pl-4 border-l-2 border-orange-400/30 space-y-3">
                {provider === "anthropic" ? (
                  <>
                    <div className="space-y-1.5"><Label>Anthropic API Key</Label>
                  ...
                  <>
                    <div className="space-y-1.5"><Label>API Base URL</Label><Input value={customBase} onChange={(e)=>setCustomBase(e.target.value)} placeholder="https://your-proxy.example.com"/></div>
                    <div className="space-y-1.5"><Label>API Key</Label><Input type="password" value={customKey} onChange={(e)=>{setCustomKey(e.target.value);setCustomKeyTouched(true);}} placeholder="your-key"/></div>
                    <div className="space-y-1.5"><Label>Model Name</Label><Input value={customModel} onChange={(e)=>setCustomModel(e.target.value)} placeholder="claude-sonnet-4-20250514"/></div>
                  </>
                )}
              </motion.div>
            )}
      ...
      <FadeIn delay={0.2}>
        <Card>
          ...
          <CardContent><div className="space-y-1.5"><Label>Max Upload Size (MB)</Label><Input type="number" value={maxUploadMb} onChange={(e)=>setMaxUploadMb(Number(e.target.value))} min={1} max={500} className="w-32"/></div></CardContent>
        </Card>
      </FadeIn>
      ...
      <FadeIn delay={0.25}>
        <Card>
          <CardHeader><CardTitle className="text-muted-foreground">Server</CardTitle></CardHeader>
          <CardContent>
